...
async def list_tasks(status: str = None) -> str:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            lines = []
            for t in tasks:
                line = f"ID: {t.id}, Title: {t.title}, Status: {t.status.value}, Priority: {t.priority.value}"
                if t.due_date:
                    line += f", Due: {t.due_date.strftime('%Y-%m-%d')}"
                lines.append(line)
            return "Tasks:\n" + "\n".join(lines)
        except Exception as e:
...
async def filter_tasks(priority: str = None, status: str = None) -> str:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            if not tasks:
                return "No tasks found with specified filters"

            lines = [
                f"ID: {t.id}, Title: {t.title}, Status: {t.status.value}, Priority: {t.priority.value}"
                for t in tasks
            ]
...
async def find_similar_task(title: str, threshold: float = 0.7):
